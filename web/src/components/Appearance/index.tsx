...
const Appearance = () => {
  ...
  const handleRandomize = useCallback(async () => {
    if (!data) return;
    try {
      const result = await Nui.post('appearance_randomize');
      if (result && result.appearanceData) {
        setData(result.appearanceData);
        setStoredData((prev: any) => prev); // keep storedData unchanged to allow comparison, but we could update settings
        if (result.appearanceSettings) {
          setAppearanceSettings(result.appearanceSettings);
        }
      }
    } catch (e) {
      console.error('Failed to randomize appearance', e);
    }
